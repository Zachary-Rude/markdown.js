...
This is a basic Markdown intepreter, written in JavaScript and CSS.
...
It is based on [Showdown.js](http://showdownjs.com/).
...
function getDefaultOpts (simple) {
  ...
  var defaultOptions = {
    ...
    underline: {
      defaultValue: false,
      description: 'Enable support for underline. Syntax is double or triple underscores: `__underline word__`. With this option enabled, underscores no longer parses into `<i>` and `<b>`',
      type: 'boolean'
    },
...
/**
 * Turn Markdown link shortcuts into HTML <a> tags.
 */
...
var simpleURLRegex  = /([*~_]+|\b)(((https?|ftp|dict):\/\/|www\.)[^'">\s]+?\.[^'">\s]+?)()(\1)?(?=\s|$)(?!["<>])/gi,
    ...
    replaceMail = function (options, globals) {
      ...
      return function (wholeMatch, b, mail) {
        ...
        return b + '<a href="' + href + '" target="_blank">' + mail + '</a>';
      };
    };
...
showdown.subParser('headers', function (text, options, globals) {
  ...
  text = text.replace(setextRegexH1, function (wholeMatch, m1) {
    ...
    var spanGamut = showdown.subParser('spanGamut')(m1, options, globals),
        ...
        hashBlock = '<h' + hLevel+ ' class="heading">' + spanGamut + '</h' + hLevel + '><hr>';
  ...
  text = text.replace(atxStyle, function (wholeMatch, m1, m2) {
    ...
    var span = showdown.subParser('spanGamut')(hText, options, globals),
        ...
        header = '<h' + hLevel + ' class="heading">' + span + '</h' + hLevel + '><hr>';
...
showdown.subParser('italicsAndBold', function (text, options, globals) {
  ...
  if (options.literalMidWordUnderscores) {
    text = text.replace(/\b___(\S[\s\S]*?)___\b/g, function (wm, txt) {
      return parseInside (txt, '<b><i>', '</i></b>');
    });
    text = text.replace(/\b__(\S[\s\S]*?)__\b/g, function (wm, txt) {
      return parseInside (txt, '<b>', '</b>');
    });
    text = text.replace(/\b_(\S[\s\S]*?)_\b/g, function (wm, txt) {
      return parseInside (txt, '<i>', '</i>');
    });
  } else {
    text = text.replace(/___(\S[\s\S]*?)___/g, function (wm, m) {
      return (/\S$/.test(m)) ? parseInside (m, '<b><i>', '</i></b>') : wm;
    });
    ...
    text = text.replace(/_([^\s_][\s\S]*?)_/g, function (wm, m) {
      ...
      return (/\S$/.test(m)) ? parseInside (m, '<i>', '</i>') : wm;
    });
  }
  ...
  if (options.literalMidWordAsterisks) {
    text = text.replace(/([^*]|^)\B\*\*\*(\S[\s\S]*?)\*\*\*\B(?!\*)/g, function (wm, lead, txt) {
      return parseInside (txt, lead + '<b><i>', '</i></b>');
    });
    text = text.replace(/([^*]|^)\B\*\*(\S[\s\S]*?)\*\*\B(?!\*)/g, function (wm, lead, txt) {
      return parseInside (txt, lead + '<b>', '</b>');
    });
    text = text.replace(/([^*]|^)\B\*(\S[\s\S]*?)\*\B(?!\*)/g, function (wm, lead, txt) {
      return parseInside (txt, lead + '<i>', '</i>');
    });
  } else {
...
To load it into a web page, use this code for the JavaScript file:

```html
<script src="https://cdn.jsdelivr.net/gh/Zachary-Rude/markdown.js/markdown.js"></script>
```

And this code for the CSS file:
```html
<link href="https://cdn.jsdelivr.net/gh/Zachary-Rude/markdown.js/markdown.css" rel="stylehseet" type="text/css" />
```
 ## __Source Code for the CSS File__
